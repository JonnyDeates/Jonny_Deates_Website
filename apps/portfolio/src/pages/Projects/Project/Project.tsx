...
const Project = ({image, title, description, link, index, currentActiveIndex, setCurrentActiveIndex}: ProjectProps) => {
  ...
  return <div className={`Project ${activeClassName} ${prevClassName} ${nextClassName} ${next2ClassName}`} onClick={handleContainerClick}
    ...
    <div className='ProjectContent' style={{display: isActive ? "unset" : "none"}}>
      <h2>{title}</h2>
      <p>{description}</p>
      <Button onClick={handleButtonClick}>Visit Site</Button>
    </div>
  </div>
}
