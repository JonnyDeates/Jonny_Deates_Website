...
export class GiveawayWheelComponent implements AfterViewInit {
  ...
  canvas: HTMLCanvasElement;
    ...
    // List of Names
    names = ['Jonny', 'Tom', 'Jeff'];

    // Color Variables
    colors = ['#FFDAB9', '#E6E6FA', '#7744ff'];
    colorsUsed = ['#FFDAB9'];
    ...
    setTimeout(() => window['$']('ul.tabs').tabs(), 100);
      ...
      checkWinner() {
          ...
          for (const sVal of this.segVal) {
              if (rot >= this.pTotal(sVal.degStart + this.rotation, sVal.degEnd + this.rotation, true)
                && rot <= this.pTotal(sVal.degStart + this.rotation, sVal.degEnd + this.rotation, false)) {
                return sVal.name;
              }
          }
      }
